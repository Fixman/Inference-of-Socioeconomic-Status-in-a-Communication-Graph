...
In Figure~\ref{ROC_multiclass} we plot the \textbf{Receiver Operating Characteristic Curve} (ROC) showing $TPR$ and $FPR$ for the set of possible values of $\tau$. We see that our methodology clearly out performs random guessing (dashed straight line). We can summarize our perfomance calculating  the \textbf{Area under the curve} (AUC) which in Figure~\ref{ROC_multiclass} is $AUC = 0.74$ not that random guessing would give as a value of $AUC = 0.50$.
